...
export default function AdminDashboard(props: Props) {
  ...
  async function deleteUser(userId: string | undefined) {
    ...
    try {
      ...
      const users = await pizzaService.getUsers(0, 10, filterUserRef.current?.value || '*');
      setUserList(users);
      setUserPage(0);
    } catch (error: any) {
      console.error('Delete error:', error); // Add this to see what's happening
                          ...
                          {["Name", "Email", "Roles", "Action"].map(
                            (header) => (
                              <th
                                key={header}
                                scope="col"
                                className="px-6 py-3 text-start text-xs font-medium"
                              >
                                {header}
                              </th>
                            )
